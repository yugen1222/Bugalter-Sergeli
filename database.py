...
async def _migrate_minus_items(db: aiosqlite.Connection) -> None:
    ...
    if "closed_at" not in columns:
        await db.execute(
            "ALTER TABLE minus_items ADD COLUMN closed_at TEXT"
        )
    if "closure_comment" not in columns:
        await db.execute(
            "ALTER TABLE minus_items ADD COLUMN closure_comment TEXT"
        )
...
async def init_db() -> None:
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.executescript(
            ...
            CREATE INDEX IF NOT EXISTS idx_minus_case_status
            ON minus_items(case_status);
            """
        )
        await db.commit()
...
async def add_minus_item(
    ...
    normalized_case_status = "closed" if case_status == "closed" else "open"
    closed_at_sql = "CURRENT_TIMESTAMP" if normalized_case_status == "closed" else "NULL"
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
            ...
            (
                ...
                closure_comment.strip() if closure_comment else None,
            ),
        )
        await db.commit()
...
async def get_open_minus_items(limit: int = 30) -> list[dict[str, Any]]:
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        cur = await db.execute(
            """
...
async def reopen_minus_item(item_id: int) -> bool:
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
            """
            UPDATE minus_items
