...
from database import (
    add_employee, add_minus_item, finish_daily_check, get_daily_report,
    get_missing_today, get_month_summary, get_or_create_daily_check,
    get_period_rows, get_user_employee, init_db, list_employees,
    reset_daily_check, set_alignment, set_has_minuses, set_shift,
    set_user_employee
)
...
def reason_keyboard() -> InlineKeyboardMarkup:
    options = [
        ("🍽 Официант не пробил", "Официант не пробил"),
        ("🏷 Неправильно оценили товар", "Неправильно оценили товар"),
        ("🗑 Ошибка списания", "Ошибка списания"),
        ("🔄 Пересорт", "Пересорт"),
        ("📥 Ошибка приёмки", "Ошибка приёмки"),
        ("✍️ Другая причина", "Другая причина"),
    ]
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=title, callback_data=f"reason:{value}")]
        for title, value in options
    ])


def product_keyboard(query: str) -> InlineKeyboardMarkup | None:
    try:
    ...
    q = query.strip().lower()
    matches = [p for p in products if q in p.lower()][:8]
    if not matches:
        return None
    ...
    rows.append([InlineKeyboardButton(text=f"Использовать: {query[:40]}", callback_data="product:typed")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
async def begin_check(message: Message, state: FSMContext) -> None:
    ...
    check_id = await get_or_create_daily_check(
        telegram_user_id=message.from_user.id,
        employee_id=employee["id"],
        employee_name=employee["name"],
        check_date=today_str(),
    )
    await reset_daily_check(check_id)
    await state.clear()
    await state.update_data(check_id=check_id, employee=employee, items=[])
...
async def shift_selected(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await set_shift(data["check_id"], shift)
...
async def alignment_answer(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.edit_text(
        "✅ Выравнивание выполнено.\n\n"
        "Отправьте фото или скриншот результата выравнивания."
    )


@router.message(Flow.waiting_alignment_photo, F.photo)
async def alignment_photo(message: Message, state: FSMContext) -> None:
    if await access_denied(message):
        return
    data = await state.get_data()
    file_id = message.photo[-1].file_id
    await set_alignment(data["check_id"], True, photo_file_id=file_id)
...
@router.message(Flow.waiting_alignment_photo)
...
async def product_search(message: Message, state: FSMContext) -> None:
    ...
    if len(query) < 2:
        ...
        return
    await state.update_data(typed_product=query)
    keyboard = product_keyboard(query)
    if keyboard:
        await message.answer("Выберите товар или используйте введённое название:", reply_markup=keyboard)
    else:
...
@router.callback_query(Flow.waiting_product, F.data.startswith("product:"))
async def product_selected(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    value = callback.data.split(":", 1)[1]
    data = await state.get_data()
    product = data["typed_product"] if value == "typed" else value
...
async def quantity(message: Message, state: FSMContext) -> None:
    ...
    if value <= 0:
        ...
        return
    await state.update_data(current_quantity=value)
    await state.set_state(Flow.waiting_reason_found)
    await message.answer("Удалось найти причину минуса?", reply_markup=yes_no("found"))


@router.callback_query(Flow.waiting_reason_found, F.data.in_({"found:yes", "found:no"}))
async def found_reason(callback: CallbackQuery, state: FSMContext) -> None:
    await callback.answer()
    found = callback.data == "found:yes"
    await state.update_data(reason_found=found)

    if not found:
        await state.update_data(
            current_reason="Причину не нашли",
        ...
        await callback.message.edit_text(
            "❌ Причина не найдена.\n\n"
            "Напишите, что уже проверили и где ещё нужно искать.\n"
            "Чтобы пропустить, отправьте знак <code>-</code>"
        )
        return

    await state.set_state(Flow.waiting_reason)
    await callback.message.edit_text("Выберите причину:", reply_markup=reason_keyboard())
...
async def reason_selected(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await state.update_data(current_reason=reason, current_status="resolved")
...
async def save_item(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        f"✅ Сохранено: <b>{item['item_name']}</b> — {item['quantity']:g} шт.\n"
...
async def add_more(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if callback.data == "more:yes":
        await state.set_state(Flow.waiting_product)
        await callback.message.edit_text("Напишите название следующего товара или первые буквы:")
        return
...
async def today_report(message: Message) -> None:
    ...
    if not employee:
        return
    report = await get_daily_report(employee["id"], today_str())
    if not report:
        await message.answer("Сегодняшняя проверка ещё не заполнена.")
        return

    check, items = report["check"], report["items"]
    lines = [
        f"📋 <b>Отчёт за {check['check_date']}</b>",
        f"Сотрудник: <b>{check['employee_name']}</b>",
        f"Смена: <b>{check['shift'] or 'не указана'}</b>",
        f"Выравнивание: {'✅' if check['alignment_done'] else '❌'}",
        f"День закрыт: {'✅' if check['completed'] else '⏳'}",
        "",
    ]
    if not items:
    ...
    for i, item in enumerate(items, 1):
        lines.append(
            f"{i}. <b>{item['item_name']}</b> — {item['quantity']:g} шт.\n"
            f"Причина: {item['reason']}\n"
            f"Ответственный: {item['responsible_name'] or 'не указан'}"
        )
    await message.answer("\n".join(lines))
